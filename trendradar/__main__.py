...
class NotificationDeliveryError(RuntimeError):
    """Raised when every configured notification channel rejects a report."""


def ensure_notification_delivered(results: Dict[str, bool]) -> None:
    """Require at least one notification channel to acknowledge delivery."""
    if any(results.values()):
        return

    failed_channels = ", ".join(results) if results else "未返回渠道结果"
    raise NotificationDeliveryError(f"通知发送失败: {failed_channels}")
...
class NewsAnalyzer:
    ...
    def _send_notification_if_needed(
        ...
        if (
            ...
            ensure_notification_delivered(results)
